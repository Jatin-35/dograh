---
title: Setup
description: Get a BotrixAI dev environment running and learn the daily commands.
---

BotrixAI is a Next.js UI (`ui/`), a FastAPI backend (`api/`), and a [Pipecat](https://github.com/pipecat-ai/pipecat)-based voice pipeline (`pipecat/`, a git submodule), backed by Postgres, Redis, and MinIO.

<Tip>
**Using Claude Code or Codex?** Install the [BotrixAI setup skill](https://github.com/Jatin-35/dograh-plugins) (`/plugin marketplace add Jatin-35/dograh-plugins`, then `/plugin install botrixai@botrixai`) and ask your agent to *"set up BotrixAI for development"* — it runs the steps below for you and verifies the stack is healthy.
</Tip>
...
1. Fork [Jatin-35/dograh](https://github.com/Jatin-35/dograh) and clone **your fork**:
...
If these steps do not work for you, please open an issue on [GitHub](https://www.botrixai.com/contact).

<AccordionGroup>
<Accordion title="Cloned Jatin-35/dograh directly instead of your fork?">
Run `bash scripts/setup_fork.sh` once. It prompts for your fork URL, points `origin` at it, and adds `upstream` as `Jatin-35/dograh`. See [Fork and upstream remotes](/contribution/reference#fork-and-upstream-remotes).
</Accordion>
...
| `sdk/` | Python/TypeScript SDKs for driving BotrixAI programmatically |
...
2. Push to your fork (`origin`) and open a pull request against `Jatin-35/dograh:main`.
3. A maintainer reviews and merges.

Bug reports and feature ideas go through [GitHub Issues](https://www.botrixai.com/contact) and [Ideas](https://www.botrixai.com/contact). Issues tagged `good first issue` are a good place to start. For questions while you're working, use the [BotrixAI Community Slack](https://www.botrixai.com/contact).
