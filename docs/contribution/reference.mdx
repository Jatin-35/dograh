...
1. Run the contributor bootstrap. It configures `origin` as your fork and `upstream` as `Jatin-35/dograh`, initializes the pipecat submodule, creates the Python venv, and copies the `.env` templates:
...
The checked-in `.devcontainer/` follows the Dev Containers specification, so it should also work in Cursor, JetBrains IDEs, and other compatible editors. Only the VS Code path is actively tested — if something breaks elsewhere, please open a [GitHub issue](https://www.botrixai.com/contact).
...
- `upstream`: `Jatin-35/dograh`, where new commits land

If you cloned `Jatin-35/dograh` directly instead of your fork, run it once (inside the devcontainer is fine):
...
upstream  https://github.com/Jatin-35/dograh.git    (fetch/push)
```

<Note>
Always push feature branches to **`origin`** (your fork), then open a pull request against `Jatin-35/dograh:main`. Never push directly to `upstream`.
</Note>
