...
- The embed snippet you copy from the dashboard is a single `<script>` tag that loads `botrixai-widget.js` **asynchronously**. The widget auto-initializes once it loads and exposes `window.DograhWidget`. Code that registers callbacks must wait for the widget to be available.
...
<script>
  ...
  window.DograhWidget.onError((err) => {
    console.error('BotrixAI error:', err.message);
  });
...
export function TalkButton() {
  ...
  useEffect(() => {
    ...
    window.DograhWidget.onError((err) => console.error('BotrixAI error:', err.message));
  }, []);
