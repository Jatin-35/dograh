...
| Field | Description |
| --- | --- |
| **Endpoint URL** | The URL BotrixAI will send the POST request to. |
...
BotrixAI sends a `POST` request with the following JSON payload:
...
```javascript
app.post("/botrixai/pre-call", async (req, res) => {
