...
When the LLM decides a transfer is needed, it calls the Call Transfer tool. BotrixAI resolves the destination, validates the transfer setup, and then starts the provider transfer.

For a successful transfer:

1. The agent calls the Call Transfer tool.
2. BotrixAI resolves the destination from either static configuration or a dynamic HTTP resolver.
3. BotrixAI optionally plays a pre-transfer message.
4. BotrixAI starts dialing the destination through the telephony provider.
5. The caller hears hold music while the destination leg is connecting.
6. When the destination answers, BotrixAI connects the caller and ends the AI agent's involvement.

The transfer is a blind transfer. BotrixAI does not currently pass conversation context to the receiving human as part of the transfer.
...
BotrixAI sends a `POST` request to your resolver endpoint before starting the transfer. Your endpoint returns the destination and may return a custom message to play before dialing.
...
- Use business logic from your backend instead of hardcoding numbers in BotrixAI

## Dynamic resolver request

The resolver request body is a flat JSON object.

BotrixAI builds it from:

- **LLM parameters**: values the agent extracts from the conversation when it calls the transfer tool
- **Preset parameters**: values BotrixAI injects from fixed values or templates such as `{{initial_context.account_id}}` or `{{gathered_context.billing_issue_type}}`
...
<Note>
BotrixAI does not send the full conversation transcript to the resolver. Add the specific values you need as LLM parameters or preset parameters.
</Note>
...
<Warning>
Follow this response shape exactly. BotrixAI reads specific keys from the resolver response. For dynamic transfer resolution, an invalid response does not fall back silently: the transfer fails before dialing.
</Warning>
...
- **Resolver URL**: HTTPS or HTTP endpoint BotrixAI calls with `POST`
- **Resolver Timeout**: How long BotrixAI waits for the resolver response
- **Resolver Wait Message**: Optional message spoken while BotrixAI waits, such as "One moment while I find the right team."
- **LLM Parameters**: Values the agent should extract from the conversation, such as `billing_issue_type` or `requested_department`
- **Preset Parameters**: Values injected by BotrixAI from context or fixed values
...
1. BotrixAI resolves the static or template destination.
2. BotrixAI plays the configured pre-transfer message, if any.
3. BotrixAI starts the provider transfer.

For dynamic transfers:

1. BotrixAI may play the resolver wait message while calling your resolver.
2. If the resolver returns `custom_message`, BotrixAI plays that message.
3. If the resolver does not return `custom_message`, BotrixAI uses the configured pre-transfer message.
4. BotrixAI starts the provider transfer.

Resolver `custom_message` overrides the configured pre-transfer message for that transfer attempt.

## Transfer timeout

Transfer timeout is the maximum time BotrixAI waits for the destination to answer after the provider transfer starts.

This timeout applies to both static and dynamic transfers.

The resolver timeout is separate. It controls only how long BotrixAI waits for your HTTP resolver before dialing starts.
...
- a preset parameter, if BotrixAI should inject it from context
