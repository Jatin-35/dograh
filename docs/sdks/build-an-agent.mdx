...
description: "Assemble a BotrixAI voice agent programmatically with the SDK and save it as a draft"
---
...
- A BotrixAI [API key](/configurations/api-keys) exported as `DOGRAH_API_KEY`
- An existing agent ID to save drafts against (create one in the BotrixAI UI or via [`POST /api/v1/workflow/create`](/api-reference/agents/create-from-template))
