...
description: "Configure the speech-to-speech, Dograh-managed, or bring-your-own-key models your BotrixAI agents use."
---
...
To configure models, open **Models** in your BotrixAI dashboard:

- **Hosted:** `https://voice-app.botrixai.com/model-configurations`
...
| **Dograh** | Use Dograh-managed LLM, voice, and transcriber models behind one BotrixAI Service Key. |
...
<Warning>
An LLM is still required when you use Speech to Speech. The realtime model handles the live voice conversation, but BotrixAI uses the LLM for analysis tasks that happen outside the live audio stream.
</Warning>

## BotrixAI

Use **BotrixAI** when you want BotrixAI to manage the model providers for you. This path uses one BotrixAI Service Key for Dograh-managed models instead of separate provider keys for LLM, Voice, and Transcriber.

![BotrixAI model configuration](../images/model-configuration-botrixai.png)
...
| **API Key** | Your BotrixAI Service Key. Create Service Keys from **Developers**. |
