---
title: "Custom Domain"
description: "Deploy BotrixAI AI with custom domain names and SSL certificates"
---

Deploy BotrixAI AI with your own custom domain name for a professional production setup. By now, you should be able to create and test a voice agent by following the previous guide to setup the platform on a remote server using [Docker](docker#option-2%3A-remote-server-deployment)
...
Custom domain deployment allows you to run BotrixAI AI with a personalized domain name (like `voice.yourcompany.com`) instead of using IP addresses. This setup includes:
...
- BotrixAI AI already running on your server via the [remote deployment](docker#option-2%3A-remote-server-deployment) guide
...
```bash
curl -o setup_custom_domain.sh https://raw.githubusercontent.com/Jatin-35/dograh/main/scripts/setup_custom_domain.sh && chmod +x setup_custom_domain.sh && sudo ./setup_custom_domain.sh
```
...
- Restart BotrixAI services through the validated startup wrapper
...
Update `.env` so the canonical remote settings use your domain — `botrixai-init` reads these to render nginx's `server_name`. Replace `voice.yourcompany.com` with your actual domain throughout:
...
```bash
#!/bin/bash
# Copy renewed certificates to botrixai certs directory
