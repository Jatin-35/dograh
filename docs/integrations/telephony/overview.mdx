...
BotrixAI AI's telephony integration system provides a unified interface for connecting with various telephony providers. The same configuration powers both outbound calls (initiated from BotrixAI) and inbound calls (received on a phone number you own).
...
<CardGroup cols={3}>
  ...
  <Card title="VoiceLink" href="/integrations/telephony/voicelink">
    Inbound, outbound and blind transfer over a single media socket
  </Card>
...
All telephony providers are configured from a single page in BotrixAI:
...
<iframe
  ...
  title="Setting up telephony configurations in BotrixAI"
...
The telephony integration in BotrixAI AI supports:
...
BotrixAI resolves the org from the webhook's account credentials and the agent from the called number's **Inbound workflow** assignment. See [Inbound Calling](/integrations/telephony/inbound) for the full setup.
...
<AccordionGroup>
  ...
  <Accordion title="Webhook signature validation failing">
    - Confirm auth tokens / API secrets match between provider and BotrixAI configuration
