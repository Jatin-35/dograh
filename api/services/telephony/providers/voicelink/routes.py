...
async def voicelink_inbound_ws(websocket: WebSocket) -> None:
    ...
    try:
        ...
        # NOTE: fixed from the upstream plugin source, which called this with
        # only 4 positional args (workflow_id, user_id, "voicelink", normalized)
        # — missing organization_id, which shifts every argument after it and
        # would crash on every inbound call. Our signature is
        # (workflow_id, user_id, organization_id, provider, normalized_data, ...).
